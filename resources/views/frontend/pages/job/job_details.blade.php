                            ...
                            @if (Auth::guard('client')->user())
                                @if ((App\Models\Admin\JobRegistration::where('client_id', Auth::guard('client')->user()->id)->count()) > 0)
                                    <div class="d-flex justify-content-center mt-4">
                                        <a href="{{route('job.apply')}}" class="common_button2 text-white bg-success">Apply</a>
                                    </div>
                                @else
                                    <div class="d-flex justify-content-center mt-4">
                                        <p>You haven't Registered for this job yet. Go to
                                            <a href="{{ route('job.registration') }}"><strong class="main_color">Job Registration</strong>
                                                </a>
                                            </p>
                                    </div>
                                @endif
